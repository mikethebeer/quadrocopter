/*******************************************************************************
  I2C Driver Functions for Static Single Instance Driver

  Company:
    Microchip Technology Inc.

  File Name:
    drv_i2c_static.c
	
  Summary:
    I2C driver impementation for the static single instance driver.

  Description:
    The I2C device driver provides a simple interface to manage the I2C
    modules on Microchip microcontrollers. This file contains implemenation
    for the I2C driver.
    
  Remarks:
    Static interfaces incorporate the driver instance number within the names
    of the routines, eliminating the need for an object ID or object handle.
    
    Static single-open interfaces also eliminate the need for the open handle.
*******************************************************************************/

//DOM-IGNORE-BEGIN
/*******************************************************************************
Copyright (c) 2013 released Microchip Technology Inc.  All rights reserved.

Microchip licenses to you the right to use, modify, copy and distribute
Software only when embedded on a Microchip microcontroller or digital signal
controller that is integrated into your product or third party product
(pursuant to the sublicense terms in the accompanying license agreement).

You should refer to the license agreement accompanying this Software for
additional information regarding your rights and obligations.

SOFTWARE AND DOCUMENTATION ARE PROVIDED AS IS WITHOUT WARRANTY OF ANY KIND,
EITHER EXPRESS OR IMPLIED, INCLUDING WITHOUT LIMITATION, ANY WARRANTY OF
MERCHANTABILITY, TITLE, NON-INFRINGEMENT AND FITNESS FOR A PARTICULAR PURPOSE.
IN NO EVENT SHALL MICROCHIP OR ITS LICENSORS BE LIABLE OR OBLIGATED UNDER
CONTRACT, NEGLIGENCE, STRICT LIABILITY, CONTRIBUTION, BREACH OF WARRANTY, OR
OTHER LEGAL EQUITABLE THEORY ANY DIRECT OR INDIRECT DAMAGES OR EXPENSES
INCLUDING BUT NOT LIMITED TO ANY INCIDENTAL, SPECIAL, INDIRECT, PUNITIVE OR
CONSEQUENTIAL DAMAGES, LOST PROFITS OR LOST DATA, COST OF PROCUREMENT OF
SUBSTITUTE GOODS, TECHNOLOGY, SERVICES, OR ANY CLAIMS BY THIRD PARTIES
(INCLUDING BUT NOT LIMITED TO ANY DEFENSE THEREOF), OR OTHER SIMILAR COSTS.
*******************************************************************************/
#include "framework/driver/i2c/drv_i2c_static.h"


static uint32_t ReadCoreTimer(void);

static uint32_t ReadCoreTimer()
{
    volatile uint32_t timer;

    // get the count reg
    asm volatile("mfc0   %0, $9" : "=r"(timer));

    return(timer);
}

#define BAUD_RATE_0         50000
#define BRG_1_TIME_0        SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2)/(2*BAUD_RATE_0)
// *****************************************************************************
// *****************************************************************************
// Section: Instance 0 static driver functions
// *****************************************************************************
// *****************************************************************************
void DRV_I2C0_Initialize(void)
{
    /* Initialize I2C0 */
    PLIB_I2C_BaudRateSet(I2C_ID_1, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), 50000);
    PLIB_I2C_StopInIdleDisable(I2C_ID_1);

    /* Low frequency is enabled (**NOTE** PLIB function logic reverted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_1);

              /* set SDA to 0 */
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_15);
              /* set SCL to 1 */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_14);
              /* set SDA to O/P */
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_15);
              /* set SCL to I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_14);
    /* Enable I2C0 */
    PLIB_I2C_Enable(I2C_ID_1);
}

void DRV_I2C0_DeInitialize(void)
{
    /* Disable I2C0 */
    PLIB_I2C_Disable(I2C_ID_1);
}

// *****************************************************************************
// Section: Instance 0 Byte Transfer Functions (Master/Slave)
// *****************************************************************************
bool DRV_I2C0_SetUpByteRead(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_1))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_1); 
        return false;
    }
	
    /* Initiate clock to receive */
    PLIB_I2C_MasterReceiverClock1Byte(I2C_ID_1);
    return true;
}

bool DRV_I2C0_WaitForReadByteAvailable(void)
{
	/* Wait for Receive Buffer Full */
    if(PLIB_I2C_ReceivedByteIsAvailable(I2C_ID_1))
       return true;
    else
       return false;
}

uint8_t DRV_I2C0_ByteRead(void)
{	
    /* Return received value */
    return (PLIB_I2C_ReceivedByteGet(I2C_ID_1));
}

bool DRV_I2C0_ByteWrite(const uint8_t byte)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_1))  
    {
        PLIB_I2C_ReceivedByteGet(I2C_ID_1);
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_1); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_1))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_1);
        return false;
    }
    
    /* Transmit byte */
    PLIB_I2C_TransmitterByteSend(I2C_ID_1, byte);
    
    return true;
}

bool DRV_I2C0_WaitForByteWriteToComplete(void)
{
    /* Check for transmit busy */
    while(PLIB_I2C_TransmitterIsBusy(I2C_ID_1));

    /* Check to see if transmit has completed */
    while(!PLIB_I2C_TransmitterByteHasCompleted(I2C_ID_1));
	
    return true;
}

bool DRV_I2C0_WriteByteAcknowledged(void)
{
    /* Check to see if transmit ACKed = true or NACKed = false */
    if (PLIB_I2C_TransmitterByteWasAcknowledged(I2C_ID_1))
       return true;
    else
       return false;
}

// *****************************************************************************
// Section: Instance 0 I2C Master Mode Functions
// *****************************************************************************
void DRV_I2C0_BaudRateSet(I2C_BAUD_RATE baudRate)
{
    /* Disable I2C0 */
    PLIB_I2C_Disable(I2C_ID_1);
    
    /* Change baud rate */
    PLIB_I2C_BaudRateSet(I2C_ID_1, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), baudRate);

    /* Low frequency is enabled (**NOTE** PLIB function revereted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_1);
 
    /* Enable I2C0 */
    PLIB_I2C_Enable(I2C_ID_1);       
}

bool DRV_I2C0_MasterBusIdle(void)
{
    if (PLIB_I2C_BusIsIdle(I2C_ID_1))
       return true;
    else
       return false;
}

bool DRV_I2C0_MasterStart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_1))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_1); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_1))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_1);
        return false;
    }

    /* Check for bus collision errors */
    if (PLIB_I2C_ArbitrationLossHasOccurred(I2C_ID_1))
    {
        return false;
    }
    
    /* Issue start */
    PLIB_I2C_MasterStart(I2C_ID_1);
       
    return true;
}

bool DRV_I2C0_WaitForStartComplete(void)
{
    /* Wait for start/restart sequence to finish  (hardware clear) */
    while(!PLIB_I2C_StartWasDetected(I2C_ID_1));
	 
    return true;
}

bool DRV_I2C0_MasterRestart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_1))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_1); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_1))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_1);
        return false;
    }
    
    /* Issue restart */
    PLIB_I2C_MasterStartRepeat(I2C_ID_1); 
       
    return true;    
}

bool DRV_I2C0_MasterStop(void)
{   
    uint32_t starttime;
	/* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_1))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_1);
        return false;
    }
    

    /* Wait 1 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()-starttime < BRG_1_TIME_0);

    /* Disable I2C */
    PLIB_I2C_Disable(I2C_ID_1);

    /* Wait 2 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_0);

    /* Set SDA as I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_15);
    /* Wait 3 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_0);
	
    PLIB_I2C_Enable(I2C_ID_1);
              /* set SDA to 0 */
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_15);
              /* set SCL to 1 */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_14);
              /* set SDA to O/P */
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_15);
              /* set SCL to I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_14);
    
    return true;
}

bool DRV_I2C0_WaitForStopComplete(void)
{
		return true;
}

void DRV_I2C0_MasterACKSend(void)
{
    /* Check if receive is ready to ack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_1) )
    {
        PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_1, true);
    }    

}

void DRV_I2C0_MasterNACKSend(void)
{
    /* Check if receive is ready to nack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_1) )
    {
       PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_1, false);
    }    
}

bool DRV_I2C0_WaitForACKOrNACKComplete(void)
{
    /* Check for ACK/NACK to complete */
    while(PLIB_I2C_ReceiverByteAcknowledgeHasCompleted(I2C_ID_1));
	 
    return true;
}



static uint32_t ReadCoreTimer(void);

static uint32_t ReadCoreTimer()
{
    volatile uint32_t timer;

    // get the count reg
    asm volatile("mfc0   %0, $9" : "=r"(timer));

    return(timer);
}

#define BAUD_RATE_1         50000
#define BRG_1_TIME_1        SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2)/(2*BAUD_RATE_1)
// *****************************************************************************
// *****************************************************************************
// Section: Instance 1 static driver functions
// *****************************************************************************
// *****************************************************************************
void DRV_I2C1_Initialize(void)
{
    /* Initialize I2C1 */
    PLIB_I2C_BaudRateSet(I2C_ID_2, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), 50000);
    PLIB_I2C_StopInIdleDisable(I2C_ID_2);

    /* Low frequency is enabled (**NOTE** PLIB function logic reverted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_2);

    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_3);
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_2);
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_3);
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_2);
    /* Enable I2C1 */
    PLIB_I2C_Enable(I2C_ID_2);
}

void DRV_I2C1_DeInitialize(void)
{
    /* Disable I2C1 */
    PLIB_I2C_Disable(I2C_ID_2);
}

// *****************************************************************************
// Section: Instance 1 Byte Transfer Functions (Master/Slave)
// *****************************************************************************
bool DRV_I2C1_SetUpByteRead(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_2))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_2); 
        return false;
    }
	
    /* Initiate clock to receive */
    PLIB_I2C_MasterReceiverClock1Byte(I2C_ID_2);
    return true;
}

bool DRV_I2C1_WaitForReadByteAvailable(void)
{
	/* Wait for Receive Buffer Full */
    if(PLIB_I2C_ReceivedByteIsAvailable(I2C_ID_2))
       return true;
    else
       return false;
}

uint8_t DRV_I2C1_ByteRead(void)
{	
    /* Return received value */
    return (PLIB_I2C_ReceivedByteGet(I2C_ID_2));
}

bool DRV_I2C1_ByteWrite(const uint8_t byte)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_2))  
    {
        PLIB_I2C_ReceivedByteGet(I2C_ID_2);
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_2); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_2))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_2);
        return false;
    }
    
    /* Transmit byte */
    PLIB_I2C_TransmitterByteSend(I2C_ID_2, byte);
    
    return true;
}

bool DRV_I2C1_WaitForByteWriteToComplete(void)
{
    /* Check for transmit busy */
    while(PLIB_I2C_TransmitterIsBusy(I2C_ID_2));

    /* Check to see if transmit has completed */
    while(!PLIB_I2C_TransmitterByteHasCompleted(I2C_ID_2));
	
    return true;
}

bool DRV_I2C1_WriteByteAcknowledged(void)
{
    /* Check to see if transmit ACKed = true or NACKed = false */
    if (PLIB_I2C_TransmitterByteWasAcknowledged(I2C_ID_2))
       return true;
    else
       return false;
}

// *****************************************************************************
// Section: Instance 1 I2C Master Mode Functions
// *****************************************************************************
void DRV_I2C1_BaudRateSet(I2C_BAUD_RATE baudRate)
{
    /* Disable I2C1 */
    PLIB_I2C_Disable(I2C_ID_2);
    
    /* Change baud rate */
    PLIB_I2C_BaudRateSet(I2C_ID_2, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), baudRate);

    /* Low frequency is enabled (**NOTE** PLIB function revereted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_2);
 
    /* Enable I2C1 */
    PLIB_I2C_Enable(I2C_ID_2);       
}

bool DRV_I2C1_MasterBusIdle(void)
{
    if (PLIB_I2C_BusIsIdle(I2C_ID_2))
       return true;
    else
       return false;
}

bool DRV_I2C1_MasterStart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_2))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_2); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_2))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_2);
        return false;
    }

    /* Check for bus collision errors */
    if (PLIB_I2C_ArbitrationLossHasOccurred(I2C_ID_2))
    {
        return false;
    }
    
    /* Issue start */
    PLIB_I2C_MasterStart(I2C_ID_2);
       
    return true;
}

bool DRV_I2C1_WaitForStartComplete(void)
{
    /* Wait for start/restart sequence to finish  (hardware clear) */
    while(!PLIB_I2C_StartWasDetected(I2C_ID_2));
	 
    return true;
}

bool DRV_I2C1_MasterRestart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_2))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_2); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_2))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_2);
        return false;
    }
    
    /* Issue restart */
    PLIB_I2C_MasterStartRepeat(I2C_ID_2); 
       
    return true;    
}

bool DRV_I2C1_MasterStop(void)
{   
    uint32_t starttime;
	/* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_2))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_2);
        return false;
    }
    

    /* Wait 1 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()-starttime < BRG_1_TIME_1);

    /* Disable I2C */
    PLIB_I2C_Disable(I2C_ID_2);

    /* Wait 2 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_1);

    /* Set SDA as I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_3);
    /* Wait 3 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_1);
	
    PLIB_I2C_Enable(I2C_ID_2);
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_3);
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_2);
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_3);
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_2);
    
    return true;
}

bool DRV_I2C1_WaitForStopComplete(void)
{
		return true;
}

void DRV_I2C1_MasterACKSend(void)
{
    /* Check if receive is ready to ack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_2) )
    {
        PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_2, true);
    }    

}

void DRV_I2C1_MasterNACKSend(void)
{
    /* Check if receive is ready to nack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_2) )
    {
       PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_2, false);
    }    
}

bool DRV_I2C1_WaitForACKOrNACKComplete(void)
{
    /* Check for ACK/NACK to complete */
    while(PLIB_I2C_ReceiverByteAcknowledgeHasCompleted(I2C_ID_2));
	 
    return true;
}



static uint32_t ReadCoreTimer(void);

static uint32_t ReadCoreTimer()
{
    volatile uint32_t timer;

    // get the count reg
    asm volatile("mfc0   %0, $9" : "=r"(timer));

    return(timer);
}

#define BAUD_RATE_2         50000
#define BRG_1_TIME_2        SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2)/(2*BAUD_RATE_2)
// *****************************************************************************
// *****************************************************************************
// Section: Instance 2 static driver functions
// *****************************************************************************
// *****************************************************************************
void DRV_I2C2_Initialize(void)
{
    /* Initialize I2C2 */
    PLIB_I2C_BaudRateSet(I2C_ID_3, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), 50000);
    PLIB_I2C_StopInIdleDisable(I2C_ID_3);

    /* Low frequency is enabled (**NOTE** PLIB function logic reverted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_3);

              /* set SDA to 0 */
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
              /* set SCL to 1 */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_8);
              /* set SDA to O/P */
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
              /* set SCL to I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_8);
    /* Enable I2C2 */
    PLIB_I2C_Enable(I2C_ID_3);
}

void DRV_I2C2_DeInitialize(void)
{
    /* Disable I2C2 */
    PLIB_I2C_Disable(I2C_ID_3);
}

// *****************************************************************************
// Section: Instance 2 Byte Transfer Functions (Master/Slave)
// *****************************************************************************
bool DRV_I2C2_SetUpByteRead(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_3))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_3); 
        return false;
    }
	
    /* Initiate clock to receive */
    PLIB_I2C_MasterReceiverClock1Byte(I2C_ID_3);
    return true;
}

bool DRV_I2C2_WaitForReadByteAvailable(void)
{
	/* Wait for Receive Buffer Full */
    if(PLIB_I2C_ReceivedByteIsAvailable(I2C_ID_3))
       return true;
    else
       return false;
}

uint8_t DRV_I2C2_ByteRead(void)
{	
    /* Return received value */
    return (PLIB_I2C_ReceivedByteGet(I2C_ID_3));
}

bool DRV_I2C2_ByteWrite(const uint8_t byte)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_3))  
    {
        PLIB_I2C_ReceivedByteGet(I2C_ID_3);
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_3); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_3))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_3);
        return false;
    }
    
    /* Transmit byte */
    PLIB_I2C_TransmitterByteSend(I2C_ID_3, byte);
    
    return true;
}

bool DRV_I2C2_WaitForByteWriteToComplete(void)
{
    /* Check for transmit busy */
    while(PLIB_I2C_TransmitterIsBusy(I2C_ID_3));

    /* Check to see if transmit has completed */
    while(!PLIB_I2C_TransmitterByteHasCompleted(I2C_ID_3));
	
    return true;
}

bool DRV_I2C2_WriteByteAcknowledged(void)
{
    /* Check to see if transmit ACKed = true or NACKed = false */
    if (PLIB_I2C_TransmitterByteWasAcknowledged(I2C_ID_3))
       return true;
    else
       return false;
}

// *****************************************************************************
// Section: Instance 2 I2C Master Mode Functions
// *****************************************************************************
void DRV_I2C2_BaudRateSet(I2C_BAUD_RATE baudRate)
{
    /* Disable I2C2 */
    PLIB_I2C_Disable(I2C_ID_3);
    
    /* Change baud rate */
    PLIB_I2C_BaudRateSet(I2C_ID_3, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), baudRate);

    /* Low frequency is enabled (**NOTE** PLIB function revereted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_3);
 
    /* Enable I2C2 */
    PLIB_I2C_Enable(I2C_ID_3);       
}

bool DRV_I2C2_MasterBusIdle(void)
{
    if (PLIB_I2C_BusIsIdle(I2C_ID_3))
       return true;
    else
       return false;
}

bool DRV_I2C2_MasterStart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_3))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_3); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_3))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_3);
        return false;
    }

    /* Check for bus collision errors */
    if (PLIB_I2C_ArbitrationLossHasOccurred(I2C_ID_3))
    {
        return false;
    }
    
    /* Issue start */
    PLIB_I2C_MasterStart(I2C_ID_3);
       
    return true;
}

bool DRV_I2C2_WaitForStartComplete(void)
{
    /* Wait for start/restart sequence to finish  (hardware clear) */
    while(!PLIB_I2C_StartWasDetected(I2C_ID_3));
	 
    return true;
}

bool DRV_I2C2_MasterRestart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_3))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_3); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_3))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_3);
        return false;
    }
    
    /* Issue restart */
    PLIB_I2C_MasterStartRepeat(I2C_ID_3); 
       
    return true;    
}

bool DRV_I2C2_MasterStop(void)
{   
    uint32_t starttime;
	/* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_3))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_3);
        return false;
    }
    

    /* Wait 1 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()-starttime < BRG_1_TIME_2);

    /* Disable I2C */
    PLIB_I2C_Disable(I2C_ID_3);

    /* Wait 2 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_2);

    /* Set SDA as I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
    /* Wait 3 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_2);
	
    PLIB_I2C_Enable(I2C_ID_3);
              /* set SDA to 0 */
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
              /* set SCL to 1 */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_8);
              /* set SDA to O/P */
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
              /* set SCL to I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_8);
    
    return true;
}

bool DRV_I2C2_WaitForStopComplete(void)
{
		return true;
}

void DRV_I2C2_MasterACKSend(void)
{
    /* Check if receive is ready to ack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_3) )
    {
        PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_3, true);
    }    

}

void DRV_I2C2_MasterNACKSend(void)
{
    /* Check if receive is ready to nack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_3) )
    {
       PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_3, false);
    }    
}

bool DRV_I2C2_WaitForACKOrNACKComplete(void)
{
    /* Check for ACK/NACK to complete */
    while(PLIB_I2C_ReceiverByteAcknowledgeHasCompleted(I2C_ID_3));
	 
    return true;
}



static uint32_t ReadCoreTimer(void);

static uint32_t ReadCoreTimer()
{
    volatile uint32_t timer;

    // get the count reg
    asm volatile("mfc0   %0, $9" : "=r"(timer));

    return(timer);
}

#define BAUD_RATE_3         50000
#define BRG_1_TIME_3        SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2)/(2*BAUD_RATE_3)
// *****************************************************************************
// *****************************************************************************
// Section: Instance 3 static driver functions
// *****************************************************************************
// *****************************************************************************
void DRV_I2C3_Initialize(void)
{
    /* Initialize I2C3 */
    PLIB_I2C_BaudRateSet(I2C_ID_4, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), 50000);
    PLIB_I2C_StopInIdleDisable(I2C_ID_4);

    /* Low frequency is enabled (**NOTE** PLIB function logic reverted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_4);

              /* set SDA to 0 */
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_7);
              /* set SCL to 1 */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_8);
              /* set SDA to O/P */
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_7);
              /* set SCL to I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_8);
    /* Enable I2C3 */
    PLIB_I2C_Enable(I2C_ID_4);
}

void DRV_I2C3_DeInitialize(void)
{
    /* Disable I2C3 */
    PLIB_I2C_Disable(I2C_ID_4);
}

// *****************************************************************************
// Section: Instance 3 Byte Transfer Functions (Master/Slave)
// *****************************************************************************
bool DRV_I2C3_SetUpByteRead(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_4))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_4); 
        return false;
    }
	
    /* Initiate clock to receive */
    PLIB_I2C_MasterReceiverClock1Byte(I2C_ID_4);
    return true;
}

bool DRV_I2C3_WaitForReadByteAvailable(void)
{
	/* Wait for Receive Buffer Full */
    if(PLIB_I2C_ReceivedByteIsAvailable(I2C_ID_4))
       return true;
    else
       return false;
}

uint8_t DRV_I2C3_ByteRead(void)
{	
    /* Return received value */
    return (PLIB_I2C_ReceivedByteGet(I2C_ID_4));
}

bool DRV_I2C3_ByteWrite(const uint8_t byte)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_4))  
    {
        PLIB_I2C_ReceivedByteGet(I2C_ID_4);
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_4); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_4))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_4);
        return false;
    }
    
    /* Transmit byte */
    PLIB_I2C_TransmitterByteSend(I2C_ID_4, byte);
    
    return true;
}

bool DRV_I2C3_WaitForByteWriteToComplete(void)
{
    /* Check for transmit busy */
    while(PLIB_I2C_TransmitterIsBusy(I2C_ID_4));

    /* Check to see if transmit has completed */
    while(!PLIB_I2C_TransmitterByteHasCompleted(I2C_ID_4));
	
    return true;
}

bool DRV_I2C3_WriteByteAcknowledged(void)
{
    /* Check to see if transmit ACKed = true or NACKed = false */
    if (PLIB_I2C_TransmitterByteWasAcknowledged(I2C_ID_4))
       return true;
    else
       return false;
}

// *****************************************************************************
// Section: Instance 3 I2C Master Mode Functions
// *****************************************************************************
void DRV_I2C3_BaudRateSet(I2C_BAUD_RATE baudRate)
{
    /* Disable I2C3 */
    PLIB_I2C_Disable(I2C_ID_4);
    
    /* Change baud rate */
    PLIB_I2C_BaudRateSet(I2C_ID_4, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), baudRate);

    /* Low frequency is enabled (**NOTE** PLIB function revereted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_4);
 
    /* Enable I2C3 */
    PLIB_I2C_Enable(I2C_ID_4);       
}

bool DRV_I2C3_MasterBusIdle(void)
{
    if (PLIB_I2C_BusIsIdle(I2C_ID_4))
       return true;
    else
       return false;
}

bool DRV_I2C3_MasterStart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_4))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_4); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_4))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_4);
        return false;
    }

    /* Check for bus collision errors */
    if (PLIB_I2C_ArbitrationLossHasOccurred(I2C_ID_4))
    {
        return false;
    }
    
    /* Issue start */
    PLIB_I2C_MasterStart(I2C_ID_4);
       
    return true;
}

bool DRV_I2C3_WaitForStartComplete(void)
{
    /* Wait for start/restart sequence to finish  (hardware clear) */
    while(!PLIB_I2C_StartWasDetected(I2C_ID_4));
	 
    return true;
}

bool DRV_I2C3_MasterRestart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_4))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_4); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_4))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_4);
        return false;
    }
    
    /* Issue restart */
    PLIB_I2C_MasterStartRepeat(I2C_ID_4); 
       
    return true;    
}

bool DRV_I2C3_MasterStop(void)
{   
    uint32_t starttime;
	/* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_4))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_4);
        return false;
    }
    

    /* Wait 1 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()-starttime < BRG_1_TIME_3);

    /* Disable I2C */
    PLIB_I2C_Disable(I2C_ID_4);

    /* Wait 2 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_3);

    /* Set SDA as I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_7);
    /* Wait 3 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_3);
	
    PLIB_I2C_Enable(I2C_ID_4);
              /* set SDA to 0 */
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_7);
              /* set SCL to 1 */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_8);
              /* set SDA to O/P */
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_7);
              /* set SCL to I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_8);
    
    return true;
}

bool DRV_I2C3_WaitForStopComplete(void)
{
		return true;
}

void DRV_I2C3_MasterACKSend(void)
{
    /* Check if receive is ready to ack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_4) )
    {
        PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_4, true);
    }    

}

void DRV_I2C3_MasterNACKSend(void)
{
    /* Check if receive is ready to nack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_4) )
    {
       PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_4, false);
    }    
}

bool DRV_I2C3_WaitForACKOrNACKComplete(void)
{
    /* Check for ACK/NACK to complete */
    while(PLIB_I2C_ReceiverByteAcknowledgeHasCompleted(I2C_ID_4));
	 
    return true;
}



static uint32_t ReadCoreTimer(void);

static uint32_t ReadCoreTimer()
{
    volatile uint32_t timer;

    // get the count reg
    asm volatile("mfc0   %0, $9" : "=r"(timer));

    return(timer);
}

#define BAUD_RATE_4         50000
#define BRG_1_TIME_4        SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2)/(2*BAUD_RATE_4)
// *****************************************************************************
// *****************************************************************************
// Section: Instance 4 static driver functions
// *****************************************************************************
// *****************************************************************************
void DRV_I2C4_Initialize(void)
{
    /* Initialize I2C4 */
    PLIB_I2C_BaudRateSet(I2C_ID_5, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), 50000);
    PLIB_I2C_StopInIdleDisable(I2C_ID_5);

    /* Low frequency is enabled (**NOTE** PLIB function logic reverted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_5);

              /* set SDA to 0 */
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_4);
              /* set SCL to 1 */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_5);
              /* set SDA to O/P */
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_4);
              /* set SCL to I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_5);
    /* Enable I2C4 */
    PLIB_I2C_Enable(I2C_ID_5);
}

void DRV_I2C4_DeInitialize(void)
{
    /* Disable I2C4 */
    PLIB_I2C_Disable(I2C_ID_5);
}

// *****************************************************************************
// Section: Instance 4 Byte Transfer Functions (Master/Slave)
// *****************************************************************************
bool DRV_I2C4_SetUpByteRead(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_5))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_5); 
        return false;
    }
	
    /* Initiate clock to receive */
    PLIB_I2C_MasterReceiverClock1Byte(I2C_ID_5);
    return true;
}

bool DRV_I2C4_WaitForReadByteAvailable(void)
{
	/* Wait for Receive Buffer Full */
    if(PLIB_I2C_ReceivedByteIsAvailable(I2C_ID_5))
       return true;
    else
       return false;
}

uint8_t DRV_I2C4_ByteRead(void)
{	
    /* Return received value */
    return (PLIB_I2C_ReceivedByteGet(I2C_ID_5));
}

bool DRV_I2C4_ByteWrite(const uint8_t byte)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_5))  
    {
        PLIB_I2C_ReceivedByteGet(I2C_ID_5);
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_5); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_5))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_5);
        return false;
    }
    
    /* Transmit byte */
    PLIB_I2C_TransmitterByteSend(I2C_ID_5, byte);
    
    return true;
}

bool DRV_I2C4_WaitForByteWriteToComplete(void)
{
    /* Check for transmit busy */
    while(PLIB_I2C_TransmitterIsBusy(I2C_ID_5));

    /* Check to see if transmit has completed */
    while(!PLIB_I2C_TransmitterByteHasCompleted(I2C_ID_5));
	
    return true;
}

bool DRV_I2C4_WriteByteAcknowledged(void)
{
    /* Check to see if transmit ACKed = true or NACKed = false */
    if (PLIB_I2C_TransmitterByteWasAcknowledged(I2C_ID_5))
       return true;
    else
       return false;
}

// *****************************************************************************
// Section: Instance 4 I2C Master Mode Functions
// *****************************************************************************
void DRV_I2C4_BaudRateSet(I2C_BAUD_RATE baudRate)
{
    /* Disable I2C4 */
    PLIB_I2C_Disable(I2C_ID_5);
    
    /* Change baud rate */
    PLIB_I2C_BaudRateSet(I2C_ID_5, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_2), baudRate);

    /* Low frequency is enabled (**NOTE** PLIB function revereted) */
    PLIB_I2C_HighFrequencyEnable(I2C_ID_5);
 
    /* Enable I2C4 */
    PLIB_I2C_Enable(I2C_ID_5);       
}

bool DRV_I2C4_MasterBusIdle(void)
{
    if (PLIB_I2C_BusIsIdle(I2C_ID_5))
       return true;
    else
       return false;
}

bool DRV_I2C4_MasterStart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_5))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_5); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_5))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_5);
        return false;
    }

    /* Check for bus collision errors */
    if (PLIB_I2C_ArbitrationLossHasOccurred(I2C_ID_5))
    {
        return false;
    }
    
    /* Issue start */
    PLIB_I2C_MasterStart(I2C_ID_5);
       
    return true;
}

bool DRV_I2C4_WaitForStartComplete(void)
{
    /* Wait for start/restart sequence to finish  (hardware clear) */
    while(!PLIB_I2C_StartWasDetected(I2C_ID_5));
	 
    return true;
}

bool DRV_I2C4_MasterRestart(void)
{
    /* Check for receive overflow */
    if ( PLIB_I2C_ReceiverOverflowHasOccurred(I2C_ID_5))  
    {
        PLIB_I2C_ReceiverOverflowClear(I2C_ID_5); 
        return false;
    }
    
    /* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_5))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_5);
        return false;
    }
    
    /* Issue restart */
    PLIB_I2C_MasterStartRepeat(I2C_ID_5); 
       
    return true;    
}

bool DRV_I2C4_MasterStop(void)
{   
    uint32_t starttime;
	/* Check for transmit overflow */
    if (PLIB_I2C_TransmitterOverflowHasOccurred(I2C_ID_5))
    {
        PLIB_I2C_TransmitterOverflowClear(I2C_ID_5);
        return false;
    }
    

    /* Wait 1 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()-starttime < BRG_1_TIME_4);

    /* Disable I2C */
    PLIB_I2C_Disable(I2C_ID_5);

    /* Wait 2 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_4);

    /* Set SDA as I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_4);
    /* Wait 3 BRG */
    starttime = ReadCoreTimer();
    while(ReadCoreTimer()- starttime < BRG_1_TIME_4);
	
    PLIB_I2C_Enable(I2C_ID_5);
              /* set SDA to 0 */
    PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_4);
              /* set SCL to 1 */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_5);
              /* set SDA to O/P */
    PLIB_PORTS_PinDirectionOutputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_4);
              /* set SCL to I/P */
    PLIB_PORTS_PinDirectionInputSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_5);
    
    return true;
}

bool DRV_I2C4_WaitForStopComplete(void)
{
		return true;
}

void DRV_I2C4_MasterACKSend(void)
{
    /* Check if receive is ready to ack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_5) )
    {
        PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_5, true);
    }    

}

void DRV_I2C4_MasterNACKSend(void)
{
    /* Check if receive is ready to nack */
    if ( PLIB_I2C_MasterReceiverReadyToAcknowledge(I2C_ID_5) )
    {
       PLIB_I2C_ReceivedByteAcknowledge (I2C_ID_5, false);
    }    
}

bool DRV_I2C4_WaitForACKOrNACKComplete(void)
{
    /* Check for ACK/NACK to complete */
    while(PLIB_I2C_ReceiverByteAcknowledgeHasCompleted(I2C_ID_5));
	 
    return true;
}



/*******************************************************************************
 End of File
*/